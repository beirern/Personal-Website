...
We can see that `j` and `i` are different. When I gave `j` the value of `i` it "copied" the value to `j` so if I do operations on `j` it is completely indepenedent from `i`. This is the same for all `primitives` in Java, `byte`, `short`, `int`, `long`, `float`, `double`, and `boolean`.
